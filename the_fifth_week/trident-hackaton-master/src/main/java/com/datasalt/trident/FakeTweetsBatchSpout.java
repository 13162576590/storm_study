...
/**
 * A Spout that emits fake tweets. It calculates a random probability
 * distribution for hashtags and actor activity. It uses a dataset of 500
 * english sentences. It has a fixed set of actors and subjects which you can
 * also modify at your own will. Tweet text is one of the random 500 sentences
 * followed by a hashtag of one subject.
 * 
 * @author pere
 */
...
public class FakeTweetsBatchSpout implements IBatchSpout {
	...
	public void open(Map conf, TopologyContext context) {
		...
		try {
			sentences = (String[]) IOUtils
					.readLines(ClassLoader.getSystemClassLoader().getResourceAsStream("500_sentences_en.txt"))
	...
	private Values getNextTweet() {
		...
		return new Values(++tweetId + "", text, author, LOCATIONS[actorIndex],
				DATE_FORMAT.format(System.currentTimeMillis()));
	}

	/**
	 * Code snippet:
	 * http://stackoverflow.com/questions/2171074/generating-a-probability-
	 * distribution Returns an array of size "n" with probabilities between 0
	 * and 1 such that sum(array) = 1.
	 */
