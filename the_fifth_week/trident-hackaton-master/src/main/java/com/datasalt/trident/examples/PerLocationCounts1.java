...
/**
 * This example shows the usage of aggregate() method for aggregating the WHOLE
 * Trident batch of Tuples.
 * <p>
 * Because we aggregate the whole batch, we produce a hashmap with the counts
 * per each location. Aggregators are useful for processing Trident's batches.
 * Note how we only use the collector at the end of the Aggregator so we don't
 * emit new Tuples for each Tuple that we process: we only emit one Tuple per
 * batch. For updating databases that's the best approach: you don't usually
 * want to overload your DB with one update per each Tuple.
 * 
 * @author pere
 */
